...
> **Deprecated.** The visual node graph was replaced by the timeline-based UI in the current version. The canonical authoring interface is the Setup tab Timeline; the Python script view is generated by `timelineToScript.js`. This document is preserved for historical reference only and does not reflect the current implementation.
